// index.ts - Deno Deploy with Rate Limiting & postgres.js
...
const RATE_LIMIT = 30; // طلب في الدقيقة
...
async function ensureTablesExist() {
  ...
  try {
    await sql`CREATE TABLE IF NOT EXISTS companies (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id TEXT, name TEXT, industry TEXT, country TEXT, employee_count INT, website TEXT, score INT, risk_level TEXT, pipeline_status TEXT, analysis JSONB, buying_signals JSONB, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`;
    await sql`CREATE TABLE IF NOT EXISTS deals (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id TEXT, company_id UUID, service_id TEXT, status TEXT, description TEXT, budget TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`;
    await sql`CREATE TABLE IF NOT EXISTS negotiations (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), deal_id UUID, actor TEXT, action TEXT, new_price INT, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`;
    dbInitialized = true;
...
console.log(`🚀 B2B Pipeline Pro v4.1.0 starting with Rate Limiting (${RATE_LIMIT} req/min)...`);
Deno.serve(handler);
