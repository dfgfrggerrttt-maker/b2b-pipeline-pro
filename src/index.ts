// index.ts - Hybrid Public & Protected B2B AI Pipeline (v4.3.0)
...
// ==================== Rate Limiting (Protects Free Endpoints) ====================
const RATE_LIMIT_MAX = 30; // 30 طلب لكل دقيقة لكل IP لحماية Gemini مجاناً
...
ensureTablesExist().catch((err) => console.warn("DB init deferred:", err.message));
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // 🌟 1. مسارات مجانية عامة للعملاء وزوار الموقع 🌟
    ...
    // تحليل بالذكاء الاصطناعي مجاناً للزوار (عام)
    ...
    // 🔒 2. مسارات محمية بـ API Key (لإدارة الصفقات والمفاوضات حصراً) 🔒

    if (!isAuthorized(req)) {
      return jsonResponse({ error: "Unauthorized: Admin API Key required for negotiation management" }, 401);
    }
